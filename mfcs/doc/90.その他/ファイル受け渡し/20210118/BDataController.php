	...
	*/
	...
	{	
		...
		if(count($dataExcel) > 0) {
			...
			$startSelectDataTime = microtime(true);

			foreach($listSave as $data) {
				$res = Cyn_Block_BD::where('ProjectID', '=', $validated['val3'])
			...
			$caculaSelectData = microtime(true) - $startSelectDataTime;
			if (isset($_COOKIE['ProcessDataSelect_Time']) && $_COOKIE['ProcessDataSelect_Time']) {
				unset($_COOKIE['ProcessDataSelect_Time']);
				setcookie("ProcessDataSelect_Time", $caculaSelectData, -1, '/');
			}else{
				setcookie("ProcessDataSelect_Time", $caculaSelectData, -1, '/');
			}
			...
			try {
				//beginTransaction
				DB::transaction(function() use ($validated, $request, $listDelete, $listSave, $listLog){
					...
					if(count($listSave) > 0) {
						...
						foreach($listSave as $data) {
							...
							if(is_null($res)) {
								...
								if($res->BD_Code != $data['BD_Code'] || $res->BData != $this->toFloat($data['BData'])) {
									$objData['BD_Code'] = $data['BD_Code'];
									$objData['BData'] = $this->toFloat($data['BData']);					
						...
						if(count($arrDataSaveInser) > 0) {
							$arrChunk = array_chunk($arrDataSaveInser, 180);
					...
					if($request->val5 == 1) {
						...
						if(count($listLog) > 0) {
							...
							if(count($arrDataLog) > 0) {
								$arrDataLogChunk = array_chunk($arrDataLog, 180);
	...
	private function processDataExcel($dataExcel, $listBDCode, $request, &$listLog, &$listSave, &$listDelete, &$errMessage) {
		...
		foreach($dataExcel as $data) {
			...
			!is_null($data[4]) && !is_null($data[5])) {
				...
				// Pattern A
				$dataA = Cyn_TosaiData::select(
					'Cyn_Plan.No AS A_No',
					'Cyn_Plan.KoteiNo AS A_KoteiNo',
					'Cyn_Plan.BD_Code AS A_BD_Code'
				)
				->join('Cyn_BlockKukaku', function($join) {
					$join->on('Cyn_TosaiData.ProjectID', '=', 'Cyn_BlockKukaku.ProjectID')
						->on('Cyn_TosaiData.OrderNo', '=', 'Cyn_BlockKukaku.OrderNo')
				...
				->where('Cyn_TosaiData.ProjectID', '=', $validated['val3'])
				->where('Cyn_TosaiData.OrderNo', '=', $validated['val4'])
				->where('Cyn_TosaiData.Name', '=', $T_Name)
				->where('Cyn_TosaiData.BKumiku', '=', $T_BKumiku)
				->where('Cyn_TosaiData.CKind', '=', $validated['val1'])
				->where('Cyn_TosaiData.WorkItemID', '!=', 0)
				
				->where('Cyn_BlockKukaku.Name', '=', $Name)
				->where('Cyn_BlockKukaku.BKumiku', '=', $BKumiku)
				->whereNull('Cyn_BlockKukaku.Del_Date')

				->where('Cyn_Plan.Kotei','=', $Kotei)
				->where('Cyn_Plan.KKumiku','=',$KKumiku)
				->whereNull('Cyn_Plan.Del_Date')
				->first();
				
				// Pattern B
				$dataB = Cyn_TosaiData::select(
					'Cyn_C_Plan.No AS B_No',
					'Cyn_C_Plan.KoteiNo AS B_KoteiNo',
					'Cyn_C_Plan.BD_Code AS B_BD_Code'
				)
				->join('Cyn_BlockKukaku', function($join) {
					$join->on('Cyn_TosaiData.ProjectID', '=', 'Cyn_BlockKukaku.ProjectID')
						->on('Cyn_TosaiData.OrderNo', '=', 'Cyn_BlockKukaku.OrderNo')
				...
				->where('Cyn_BlockKukaku.Name', '=', $T_Name)
				->where('Cyn_BlockKukaku.BKumiku', '=', $T_BKumiku)
				->whereNull('Cyn_BlockKukaku.Del_Date')

				->where('Cyn_C_BlockKukaku.Name', '=', $Name)
				->where('Cyn_C_BlockKukaku.BKumiku', '=', $BKumiku)
				->whereNull('Cyn_C_BlockKukaku.Del_Date')

				->where('Cyn_C_Plan.Kotei','=', $Kotei)
				->where('Cyn_C_Plan.KKumiku','=',$KKumiku)
				->whereNull('Cyn_C_Plan.Del_Date')
				->first();
					...
					for ($i = $indexBegin; $i < $indexEnd; $i += 2) { 
						if (!is_null($data[$i])) {
							...
							if($message != '') {
								...
								$arraySaveTemp['A_No'] = is_null($dataA) ? null : $dataA->A_No;
								$arraySaveTemp['A_KoteiNo'] = is_null($dataA) ? null : $dataA->A_KoteiNo;
								$arraySaveTemp['A_BD_Code'] = is_null($dataA) ? null : $dataA->A_BD_Code;
								$arraySaveTemp['B_No'] = is_null($dataB) ? null : $dataB->B_No;
								$arraySaveTemp['B_KoteiNo'] = is_null($dataB) ? null : $dataB->B_KoteiNo;
								$arraySaveTemp['B_BD_Code'] = is_null($dataB) ? null : $dataB->B_BD_Code;
