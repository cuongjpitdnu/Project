...
<script>
	$(function() {
		...
		$('.rowdetail').on('click', function(e) {
			...
			var url = '{{ url("/") }}/{{ $menuInfo->KindURL }}/{{ $menuInfo->MenuURL }}/indexdetail';
			url += '?cmn1={{ valueUrlEncode($menuInfo->KindID) }}&cmn2={{ valueUrlEncode($menuInfo->MenuID) }}';
			url += '&val1='+val1;
			url += '&val3='+val3;
			window.location.href = url;
		...
		<div class="row">
			<div class="col-sm-12">
				<table class="table table-row">
					<thead>
						<th class="text-center">@sortablelink('fld1', trans('mstkoteistrp.sortable.fld1'))</th>
						<th class="text-center">@sortablelink('fld2', trans('mstkoteistrp.sortable.fld2'))</th>
						<th class="text-center">@sortablelink('fld3', trans('mstkoteistrp.sortable.fld3'))</th>
						<th class="text-center">@sortablelink('fld4', trans('mstkoteistrp.sortable.fld4'))</th>
					...
					<tbody>
						@foreach($rows as $row)
							<tr>
								...
								<td class="real-space align-middle">{{ (int)$row['fld3'] === config('system_const.c_kind_chijyo') ?
								...
								<td class="real-space align-middle">{{ (int)$row['fld4'] === 0 ? '有効' : '無効' }}</td>
								<td class="text-center align-middle">
									<button type="button" class="rowdetail {{ config('system_const.btn_color_rowdetail') }}">
								...
								<td class="text-center align-middle">
									@if (!$menuInfo->IsReadOnly)
										<button type="button" class="edit {{ config('system_const.btn_color_rowedit') }}"
											fld1="{{ valueUrlEncode($row['fld1']) }}" fld3="{{ valueUrlEncode($row['fld3']) }}">
