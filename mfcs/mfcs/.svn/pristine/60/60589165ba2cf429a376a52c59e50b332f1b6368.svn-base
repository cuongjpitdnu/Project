	...
	<div class="col-xs-12">
		...
		<div class="row">
			<div class="col-sm-12">
				<table class="table">
					<thead>
						<tr>
							...
							<th class="text-center">@sortablelink('fld4', '有効')</th>
					...
					<tr>
						<td class="real-space align-middle">{{ $bData['fld1'] }}</td>
						<td class="real-space align-middle">{{ $bData['fld2'] }}</td>
						<td class="real-space align-middle">{{ $bData['fld3'] }}</td>
						<td class="real-space align-middle">{{ $bData['fld4'] ? '有効' : '無効' }}</td>
						<td class="text-center align-middle">
							@if (!$menuInfo->IsReadOnly)
								<button type="button" class="edit {{ config('system_const.btn_color_rowedit') }}"
								value="{{ valueUrlEncode($bData['fld1']) }}">
				...
				{{ $bDatas->appends(\Request::except('page'))->render() }}
