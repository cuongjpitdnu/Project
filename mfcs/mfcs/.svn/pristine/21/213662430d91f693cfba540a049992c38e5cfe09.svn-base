...
<ul>
	...
	?>
	<li name="orgitem" item_id="{{ valueUrlEncode($grpID) }}" item_name="{{ $mstItem['name'] }}" s_date="{{ valueUrlEncode($mstItem['sdate']) }}" data-jstree='{"selected":{{ $selected }}, "opened":{{ $opened }}, "icon":"{{ $icon }}"}'>{{ $mstItem['name'] }}
