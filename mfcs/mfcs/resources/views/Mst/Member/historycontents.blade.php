	...
	<div class="col-xs-12">
		...
		<div class="row align-items-center">
			<div class="col-xs-1 text-left m-2 p-2 rounded border">
				■　人員マスタ履歴@if($target === 'show')参照@elseif($target === 'create')登録@elseif($target === 'edit')更新@endif
			</div>
		</div>
		...
		<form action="{{ url('/') }}/{{ $menuInfo->KindURL }}/{{ $menuInfo->MenuURL }}/historysave" method="POST" id="mainform">
			...
			<input type="hidden" id="searchpage" name="searchpage" value="{{ $request->searchpage }}">
			<input type="hidden" id="searchsort" name="searchsort" value="{{ $request->searchsort }}">
			<input type="hidden" id="searchdirection" name="searchdirection" value="{{ $request->searchdirection }}">
			...
			<input type="hidden" id="val201" name="val201" value="{{ $request->val201 }}">
			<input type="hidden" id="val202" name="val202" value="{{ $request->val202 }}">
			...
			<table class="table table-borderless">
				...
				<tr>
					...
					<td>
						<input type="text" name="val310" value="{{ old('val310', @$memHist['val310']) }}" class="text-right"
						{{ $target === "show" ? 'disabled="disabled"' : '' }} autocomplete="off" maxlength="10">
					</td>
					<td class="p-0 align-middle">
						<span class="col-xs-1 p-1">
