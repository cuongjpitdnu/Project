...
public class DebugForm : Form
{
    ...
    public DebugForm(ModbusMaster _master, int _devno, Sensor _ts)
    {
        InitializeComponent();
        Master = _master;
        TargetSensor = _ts;
        ...
        dataGridView.RowCount = TargetSensor.DebugMaps.Count;
        for (int i = 0; i < dataGridView.Rows.Count; i++)
        {
            dataGridView[0, i].Value = TargetSensor.DebugMaps[i].Title.ToString();
