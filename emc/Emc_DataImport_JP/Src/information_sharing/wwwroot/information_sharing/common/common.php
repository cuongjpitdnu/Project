<?php
	...
	*/
	function fncWriteLogToDb($intLogLevel, $intLogPattern, $strMsg){
		...
		$intMaxLen = 3500;
		if(mb_strlen($strMsg)>$intMaxLen){
			$output = preg_split('//u', $strMsg, null, PREG_SPLIT_NO_EMPTY);
			
			$strContent = date('Y/m/d H:i:s') .' ' .$strLogLevel .' ';
			for ($i = 0; $i < $intMaxLen; $i++) {
				$strContent .= $output[$i];
			}
	...
	const MAIL_SMTP_AUTH = false;
	const MAIL_SMTP_AUTO_TLS = false;
	...
	function fncSendMail($arrUser = array(), $strTitile = '', $strHtmlBody = '', $noneHtmlBody = '') {
		...
		try {
			...
			$mail->SMTPAutoTLS = MAIL_SMTP_AUTO_TLS;
	...
	function getDataMUserSendMail($strField, $strScreenName = '') {
		try {
			$strSQL = ' SELECT '
					...
					. '     mu.language_type, '
					. '     mu.mail_address '
					//▼2020/06/11 KBS S.Tasaki 関空ユーザ識別のためにSQL条件を追加
					. '   , mg.admin_flag '
					//▲2020/06/11 KBS S.Tasaki
					. ' FROM '
					. '     m_user AS mu '
					//▼2020/06/11 KBS S.Tasaki 関空ユーザ識別のためにSQL条件を追加
					. '     INNER JOIN m_company mc ON mu.company_no = mc.company_no '
					. '     INNER JOIN m_group mg ON mc.group_no = mg.group_no '
					//▲2020/06/11 KBS S.Tasaki
					. ' WHERE ';
    ...
    *				: 2020/05/27 T.Masuda   翻訳欄の文字数チェックを行わないように変更
    */
    function fncTranInputCheck($strTitleOriginal, $strContentOriginal,
        ...
        if($intButtonKind == 1 && ($intManualInput == 1 || ($intManualInput == 0
             ...
             //▼2020/05/27 KBS T.Masuda 翻訳欄の文字数チェックを削除
             /*
             // タイトル（翻訳）文字数
             ...
             if($strTranslate == 0) {
                 ...
                 //タイトル（翻訳）
                 $strCheckTitleTranslation = $strTitleTranslation;
                 $strCheckTitleTranslation = str_replace('“', '', $strCheckTitleTranslation);
                 $strCheckTitleTranslation = str_replace('”', '', $strCheckTitleTranslation);
                 if(!mb_detect_encoding($strCheckTitleTranslation, 'ASCII', true)){
                        $arrErrorMsg[] =  $arrTextTranslate['PUBLIC_MSG_033'];
                 }
                 
                 // 内容（翻訳）
                 $strCheckContentTranslation = $strContentTranslation;
                 $strCheckContentTranslation = str_replace('“', '', $strCheckContentTranslation);
                 $strCheckContentTranslation = str_replace('”', '', $strCheckContentTranslation);
                 if(!mb_detect_encoding($strCheckContentTranslation, 'ASCII', true)){
