...
// check if isset txtUserId to process, if don't have continue load view login.php
if(isset($_POST['txtUserId'])){
    ...
    // If have error, break to page view login.php
    if(isset($_SESSION['LOGIN_OTP_ERROR'])){
    ...
    if($arrUserData && $arrUserData[0]['user_id'] == $strUserID 
       && $arrUserData[0]['password'] == $strPassword){
...
function fncErrorSendSMS($intCodeError,$intType){
    ...
    // with type different 6 will return to login.php. if type 6 will return error only
    if($intType != 6){
        $_SESSION['LOGIN_OTP_ERROR'] = $strError;
        header('Location: login.php');
        exit;
...
function fncRakuService($strUserID, $strPassword,$test_type,$otp = NULL,$phone_sn = ''){
    ...
    if($ret){
        ...
        // check type
        if($test_type == 1){
            fncWriteLog(LogLevel['Error'], LogPattern['Error'], DISPLAY_NAME.' らく認ユーザエラー '.$strLogRakunin);
        }else if($test_type == 4){
