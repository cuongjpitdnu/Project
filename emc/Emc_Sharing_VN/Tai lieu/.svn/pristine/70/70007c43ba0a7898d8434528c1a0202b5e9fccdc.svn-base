...
Public Class AppManager
    ...
    Public Shared logErrFolderPath As String
    ...
    Public Shared strBuf As String
                    ...
                    If intIdx > 0 Then
                        ...
                        If mymail.Body.Multiparts.Length = 0 Then
                            ...
                            If StrConv(strEncoding, VbStrConv.Uppercase) = StrConv("base64", VbStrConv.Uppercase) Then
                                ...
                                'Dim c As Char() = mymail.Body.Text.ToCharArray()
                                'Dim b As Byte() = Convert.FromBase64CharArray(c, 0, c.Length)
                                'Dim s As String = Encoding.GetEncoding(strBuf).GetString(b)
                        ...
                        Else
                            ...
                            If StrConv(strEncoding, VbStrConv.Uppercase) = StrConv("base64", VbStrConv.Uppercase) Then
                                ...
                                'Dim c As Char() = part1.Body.Text.ToCharArray()
                                'Dim b As Byte() = Convert.FromBase64CharArray(c, 0, c.Length)
                                'Dim s As String = Encoding.GetEncoding(strBuf).GetString(b)
                        ...
                        If ((mymailSubject.Count = 0 Or subjectMail.Trim = "") And String.IsNullOrEmpty(mymailbody.Trim)) Then
                            ...
                            'ElseIf subjectMail.Trim = "" And String.IsNullOrEmpty(mymailbody.Trim) Then
                            '    'Write err log
                            '    Dim errMessLogEmail As String = DateTime.Now.ToString(clsConst.cstrDateTimeFomatShow) & " " & "EmcDataImport.vb" & " " & "EMCDataImportProcessing" & " " & "件名および本文に値がありませんでした。"
                            '    xWriteLog(logErrFolderPath, fileErrNameLog, errMessLogEmail)
                            '    '---------------'
                        ...
                        contentsJPN = mymailbody.Replace(vbCr, "").Replace(vbLf, "")
    ...
    Private Shared Function SendMail(subject As String, bodyMail As String, toMail As String) As Boolean
        ...
        Try
            Using mail As New MailMessage()
                ...
                mail.Body = bodyMail
                mail.BodyEncoding = Text.Encoding.GetEncoding(strBuf)
    ...
    Private Shared Sub SendMail()
        Try
            ...
            Smtp_Server.Host = "192.168.1.197"

            e_mail = New MailMessage()
            e_mail.From = New MailAddress("test@cuongnm.vn")
            e_mail.To.Add("chien@cuongnm.vn")
            e_mail.Subject = ""
            e_mail.IsBodyHtml = True
            e_mail.Body = "TEST Title NULL"
    ...
    Public Shared Function DecodeBase64(input As String, chatset As String) As String
        Return System.Text.Encoding.GetEncoding(chatset).GetString(Convert.FromBase64String(input))
    End Function
