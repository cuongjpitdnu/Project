...
Public Class clsEmcDataImport
            ...
            If sqlDataReader.HasRows Then
                ...
                While sqlDataReader.Read()
                    ...
                    If reg_date = "" Or res <> reg_date Then
                        swWebLogFile = New StreamWriter(My.Settings.WEB_LOG_FOLDER & "\" & res & ".txt")
                        swWebLogFile.WriteLine(sqlDataReader("CONTENT").Trim().TrimEnd(vbCrLf))
                        swWebLogFile.Close()

                        reg_date = res
                    Else
                        swWebLogFile = New StreamWriter(My.Settings.WEB_LOG_FOLDER & "\" & res & ".txt", True)
                        swWebLogFile.WriteLine(sqlDataReader("CONTENT").Trim().TrimEnd(vbCrLf))
                        swWebLogFile.Close()
                    End If
    ...
    Private Shared Sub DeleteInquiryData()
        ...
        Try
            ...
            mClsDb = New clsDbAccess
            mClsDb.Open(mstrConnectionSql)
            sqlDataReader = mClsDb.GetInquiryData()
    ...
    Private Shared Function GetListMailAddress() As List(Of String)
        ...
        Try
            ...
            dtMailAddress = mClsDb.GetAllMailAddress()
    ...
    Private Shared Function CreateKeyAnnounceNo() As Integer
        ...
        Try
            ...
            mClsDb.Open(mstrConnectionSql)
            result = mClsDb.GetKeyForAnnounceNo()
    ...
    Private Shared Function DecodeQuotedPrintable(input As String, chatset As String) As String
        ...
        For Each m As Match In matches
            ...
            While intIndex < bytes.Length
                Dim strHex As String = m.Value.Substring(intIndex * 3 + 1, 2)
