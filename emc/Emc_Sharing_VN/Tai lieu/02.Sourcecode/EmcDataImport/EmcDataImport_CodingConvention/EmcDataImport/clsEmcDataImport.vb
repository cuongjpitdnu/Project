    ...
    Private Shared Sub ExportWebLogFile()
        ...
        Try
            ...
            If objSqlDataReader.HasRows Then
                ...
                Dim files() As String = IO.Directory.GetFiles(My.Settings.SHARE_FOLDER + "\" + My.Settings.WEB_LOG_FOLDER)
                For Each file As String In files
                    ...
                    If dtCreateTime <= dtOneYearAgo.AddDays(-1) Then fi.Delete()
                Next
                ' ----------------------------------------------------------------------------   
                ...
                While objSqlDataReader.Read()
                    ...
                    If strReg_date = "" Or strRes <> strReg_date Then
                        '▽2020/04/22 Cuong --- S --- に変更
                        If File.Exists(My.Settings.SHARE_FOLDER + "\" + My.Settings.WEB_LOG_FOLDER & "\" & strRes & ".txt") Then
                            objStreamWriteWebLogFile = New StreamWriter(My.Settings.SHARE_FOLDER + "\" + My.Settings.WEB_LOG_FOLDER & "\" & strRes & ".txt", True)
                            objStreamWriteWebLogFile.WriteLine(objSqlDataReader("CONTENT").Trim().TrimEnd(vbCrLf))
                            objStreamWriteWebLogFile.Close()
                        Else
                            objStreamWriteWebLogFile = New StreamWriter(My.Settings.SHARE_FOLDER + "\" + My.Settings.WEB_LOG_FOLDER & "\" & strRes & ".txt")
                            objStreamWriteWebLogFile.WriteLine(objSqlDataReader("CONTENT").Trim().TrimEnd(vbCrLf))
                            objStreamWriteWebLogFile.Close()
                        End If
                        '△2020/04/22 Cuong --- E --- に変更
