...
namespace GPCommon
{
    public class LunarCalendar
    {
        ...
        public LunarCalendar(DateTime dtValue)
        {
            SetLunarDateFromSolarDate(dtValue);
        }

        public LunarCalendar(int intSDay, int intSMonth, int intSYear)
        {
        ...
        public LunarCalendar(int dd, int mm, int yy, ENUM_LEAP_MONTH leap, int jd)
        {
            this.intLunarDay = dd;
        ...
        List<LunarCalendar> decodeLunarYear(int yy, int k)
        {
            ...
            List<LunarCalendar> ly = new List<LunarCalendar>();
            ...
            if (leapMonth == 0)
            {
                for (mm = 1; mm <= 12; mm++)
                {
                    ly.Add(new LunarCalendar(1, mm, yy, ENUM_LEAP_MONTH.NormalMonth, currentJD));
            ...
            else
            {
                ...
                ly.Add(new LunarCalendar(1, leapMonth, yy, ENUM_LEAP_MONTH.LeapMonth, currentJD));
        ...
        public List<LunarCalendar> GetYearInfo(int intYYYY)
        {
        ...
        LunarCalendar findLunarDate(int jd, List<LunarCalendar> ly)
        {
            if (jd > LAST_DAY || jd < FIRST_DAY || ly[0].intJd > jd)
            {
                return new LunarCalendar(0, 0, 0, 0, jd);
            }
            var i = ly.Count - 1;
            while (jd < ly[i].intJd)
            {
            ...
            LunarCalendar ret = new LunarCalendar(ly[i].intLunarDay + off, ly[i].intLunarMonth, ly[i].intLunarYear, ly[i].LunarMonthType, jd);
        ...
        {
            DateTime dtSolarDate = new DateTime(intSYear, intSMonth, intSDay);
            SetLunarDateFromSolarDate(dtSolarDate);
        }

        public void SetLunarDateFromSolarDate(DateTime dtSlDate)
        {
            LunarCalendar objLC = GetLunarDate(dtSlDate);
        ...
        public LunarCalendar GetLunarDate(DateTime dtDate)
        {
        ...
        /// <returns></returns>
        public LunarCalendar GetLunarDate(int solarDD, int solarMM, int solarYYYY)
        {
            List<LunarCalendar> ly;
            int jd;
            if (solarYYYY < 1200 || 2199 < solarYYYY)
            {
                return new LunarCalendar(0, 0, 0, 0, 0);
            }
            ...
            LunarCalendar objLD = findLunarDate(jd, ly);
        ...
        public DateTime GetSolarDate(int dd, int mm, int yyyy)
        {
            ...
            List<LunarCalendar> ly = GetYearInfo(yyyy);
            LunarCalendar lm = ly[mm - 1];
        ...
        public DateTime GetSolarDate(int dd, int mm, int yyyy, bool blnLeapMonth)
        {
            ...
            if (ly.Count == 13)//Năm là năm nhuận
            {
                if (blnLeapMonth)//Trường hợp tháng nhuận
                {
                    if (lm.intLunarMonth == mm)
                    {
                        lm = ly[mm];
                ...
                else //Trường hợp tháng lớn hơn tháng nhuận 
                {
                    int leapMonth = GetLeapMonth(yyyy);
                    if (mm > leapMonth)
                    {
                        lm = ly[mm];
                    }
        ...
        /// <param name="yy"></param>
        /// <returns></returns>
        public List<LunarCalendar> GetMonth(int mm, int yy)
        {
            List<LunarCalendar> ly1, ly2;
            int tet1, jd1, jd2;
            int mm1, yy1, i;
            if (mm < 12)
            {
            ...
            tet1 = ly1[0].intJd;
            List<LunarCalendar> result = new List<LunarCalendar>();
            if (tet1 <= jd1)
        ...
        public int GetLunarMonthDays(int lunarMM, int lunarYYYY, bool blnLeap)
        {
            ...
            LunarCalendar objLunar = GetLunarDate(dtSolar.AddDays(1));

            if (objLunar.intLunarMonth == lunarMM) return 30;
            return 29;
        }

        //public string GetDayName(LunarCalendar lunarDate)
        //{
        //	if (lunarDate.intLunarDay == 0)
        //	{
        //		return "";
        //	}
        //	LunarCalendar cc = GetCanChi(lunarDate);
        ...
        //public LunarCalendar GetCanChi(LunarCalendar lunar)
        //{
        ...
        private string getDayInfo(LunarCalendar lunar)
        {
            string s;
        ...
        public string GetLunarDateInfo(LunarCalendar lunarDate)
        {
        ...
        {
            LunarCalendar currentLunarDate = GetLunarDate(dtSunDate.Day, dtSunDate.Month, dtSunDate.Year);
        ...
        {
            var dayString = intLunarDay > 0 ? intLunarDay.ToString() : UNKNOWN;
            var monthString = intLunarMonth > 0 ? intLunarMonth.ToString() : UNKNOWN;
            var yearString = intLunarYear > 0 ? intLunarYear.ToString() : UNKNOWN;
            dayString = dayString.PadLeft(2, '0');
            monthString = monthString.PadLeft(2, '0') + (LunarMonthType == ENUM_LEAP_MONTH.LeapMonth ? " nhuận" : "");
